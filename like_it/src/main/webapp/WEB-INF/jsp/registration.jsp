...
<fmt:message bundle="${loc}" key="local.locbutton.name.en"
	var="en_button" />
<fmt:message bundle="${loc}" key="local.main_text.registration_text"
...
<fmt:message bundle="${loc}" key="local.error.login_error"
	var="login_error" />
<fmt:message bundle="${loc}" key="local.error.not_all_error"
	var="not_all_error" />
<fmt:message bundle="${loc}" key="local.error.password_error"
	var="password_error" />
</head>
<body>
	...
	<form action="Controller" method="post">
		...
		<figure class="col-4 col-m-12 account_photo center"> <c:if
			...
			<p>
				<img src="jsp/content/anonim.jpg" alt="фотография пользователя"
					class="big_photo">
			</p>
		...
		<c:if test="${sessionScope.login_error == 'yes'}">
			<label class="input_error" id="login_input_error" for="login"><c:out
					value="${login_error}" /></label>
		</c:if> <c:if test="${sessionScope.not_all_error == 'yes'}">
			<label class="input_error" id="login_input_error" for="login"><c:out
					value="${not_all_error}" /></label>
		</c:if> <c:if test="${sessionScope.password_error == 'yes'}">
		...
		<c:if test="${sessionScope.change != 'yes'}">
			<c:if test="${sessionScope.status == 'admin'}">
				<div class="line">
					<label class="input_label" for="status"><c:out
							value="${status}" /></label> <select id="status" name="status">
			...
			<div class="line">
				<label class="input_label" for="password2"><c:out
						value="${password2}" /></label> <input id="password2" name="password2"
					type="password">
			</div>
		</c:if> </section>
